"""Fake aiohttp session/response doubles shared by the Polygon test
modules. No real socket is ever opened -- PolygonClient's session is
injected via its `session=` constructor argument, so these fakes are
all any test needs to exercise the request/response/error-mapping
logic. Mirrors tests/unit/market_data/sahmk/_fakes.py exactly."""
