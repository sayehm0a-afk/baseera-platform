...
def ledger():
    ...
    metadata = MetaData()
    snapshots = Table(
        "decision_v2_snapshots", metadata,
    ...
    with engine.begin() as connection:
        stamps = ["2026-09-08T20:59:59", "2026-09-08T21:00:00", "2026-09-09T20:59:59", "2026-09-09T21:00:00"]
        for identifier, stamp in enumerate(stamps, 1):
            connection.execute(snapshots.insert().values(
                id=identifier, decision="BUY_CANDIDATE", decision_timestamp=datetime.fromisoformat(stamp),
            ))
        connection.execute(outcomes.insert().values(
            id=1, decision_v2_snapshot_id=2, status="PARTIAL", entry_triggered=True, is_independent_signal=True,
        ))
