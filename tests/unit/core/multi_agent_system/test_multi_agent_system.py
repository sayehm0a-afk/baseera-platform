...
async def test_supervisor_agent_initialize_llm_client_logging(caplog):
    ...
    with caplog.at_level(logging.INFO):
        ...
        for record in caplog.records:
            if record.name == "src.core.multi_agent_system.supervisor_agent" and re.match(
                r"LLM client for SupervisorAgent .* set to None \(or specialized later\).", record.message
            ):
                found_log = True
                break
