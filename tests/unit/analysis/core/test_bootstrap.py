...
def test_all_default_engines_are_registered():
    ...
    assert "composite_analysis" in names
...
def test_composite_engine_factory_produces_a_composite_intelligence_engine():
    spec = DEFAULT_ENGINE_REGISTRY.get("composite_analysis")
    engine = spec.engine_factory()
    assert isinstance(engine, CompositeIntelligenceEngine)
...
def test_register_default_engines_is_idempotent():
    ...
    assert names.count("composite_analysis") == 1
