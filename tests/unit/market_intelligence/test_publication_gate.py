...
def test_calibrated_probability_exactly_at_the_60_percent_floor_passes():
    """2026-09-17 explicit product decision: the floor is 0.60 (lowered
    from 0.70 after real production evidence showed 0.70 published zero
    candidates all day) -- a value that would have comfortably passed
    the old 0.35 default must now only pass at or above 0.60."""
    outcome = make_outcome(decision=make_decision(recommendation=Recommendation.BUY, expected_return_pct=5.0, risk_reward_ratio=2.0))
...
def test_calibrated_probability_just_below_the_60_percent_floor_is_rejected():
    """The exact scenario the 0.60 floor exists for: a calibrated
    probability (0.55) within the real 54-61% distribution observed in
    production, but below the floor, must be rejected."""
    outcome = make_outcome(decision=make_decision(recommendation=Recommendation.BUY, expected_return_pct=5.0, risk_reward_ratio=2.0))
    evaluation = evaluate_publication(outcome, calibrated_success_probability=0.55)
    assert evaluation.status is PublicationStatus.REJECTED
    gate = next(g for g in evaluation.gates if g.name == "confidence_calibration")
    assert gate.status is GateStatus.FAIL
