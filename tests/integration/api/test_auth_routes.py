...
def test_login_with_wrong_password_returns_401(client: TestClient, db_session):
    ...
    response = client.post(
        "/api/v1/auth/login", json={"email": "wrongpass@example.com", "password": "totally-wrong"}
    )
...
def test_revoke_unknown_session_returns_404(client: TestClient, db_session):
    ...
    response = client.delete("/api/v1/auth/sessions/999999")
...
def test_forgot_password_and_reset_changes_password_and_revokes_sessions(client: TestClient, db_session):
    ...
    reset_response = client.post(
        "/api/v1/auth/reset-password", json={"token": raw_token, "new_password": "brand-new-password"}
    )
    ...
    old_login = client.post("/api/v1/auth/login", json={"email": "reset@example.com", "password": "old-password"})
...
def test_logout_all_kills_a_still_live_access_token_on_another_device_too(client: TestClient, db_session):
    ...
    client.post("/api/v1/auth/login", json={"email": email, "password": "s3cret-password"})
