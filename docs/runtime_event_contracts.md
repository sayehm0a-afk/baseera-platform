...
### 2. بنية الحدث الأساسية (Base Event Structure)
...
### 3. أمثلة على عقود الأحداث

#### 3.1 `TaskCreated` Event

يتم نشره عندما يتم إنشاء مهمة جديدة في النظام.
...
#### 3.2 `AgentActivated` Event

يتم نشره عندما يتم تفعيل وكيل بنجاح.
...
#### 3.3 `ToolExecuted` Event

يتم نشره بعد تنفيذ أداة بنجاح أو فشل.
...
### 4. إدارة إصدارات العقود (Contract Versioning)
...
## مخططات الرسائل (Message Schemas)
...
### 1. بنية الأمر الأساسية (Base Command Structure)
...
### 2. بنية الطلب/الرد الأساسية (Base Request/Response Structure)

تُستخدم للاتصالات المتزامنة أو التي تتطلب استجابة مباشرة.

#### 2.1 بنية الطلب (Request Structure)
...
#### 2.2 بنية الرد (Response Structure)
...
## نموذج الجدولة (Scheduling Model)
...
### 2. خصائص المهام (Task Properties)
...
### 4. التعامل مع حالات الفشل (Failure Handling)
...
### 1. مبادئ استراتيجية إعادة المحاولة
...
### 2. أنواع استراتيجيات التراجع (Backoff Strategies)
...
### 2. الذاكرة قصيرة المدى (Short-term Memory Contracts)

تُستخدم لتخزين السياق الحالي للمحادثة أو المهمة (Context Window).

#### 2.1 واجهة `IShortTermMemory`
...
### 3. الذاكرة طويلة المدى (Long-term Memory Contracts)

تُستخدم لتخزين المعرفة الدائمة، الخبرات السابقة، والبيانات التاريخية.

#### 3.1 واجهة `ILongTermMemory`
...
## عقود واجهة برمجة التطبيقات (API Contracts)
...
### 3. أمثلة على عقود API

#### 3.1 `POST /api/v1/tasks` - إنشاء مهمة جديدة
...
#### 3.2 `GET /api/v1/tasks/{task_id}` - استرجاع حالة مهمة
