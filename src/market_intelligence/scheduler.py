...
class IntervalMarketIntelligenceScheduler:
    ...
    def _quota_allows_a_new_cycle(self) -> bool:
        ...
        PROPOSED (2026-09-10, NOT YET APPLIED): now reads
    ...
    async def _stage2_runner(
        ...
        the same-named PROPOSED request-priority tier below). `caller` is
        accepted only to satisfy `StageTwoRunner`'s shape (unused --
        nothing here logs it; `_run_one_bounded_background_cycle` is the
        caller that actually uses it, for its own manually-triggered
        routes).

        PROPOSED (2026-09-10, NOT YET APPLIED): runs under
        priority_scope(MARKET_SCAN) (the request_priority.py tier
        imported above -- distinct from operation_scope.py's
