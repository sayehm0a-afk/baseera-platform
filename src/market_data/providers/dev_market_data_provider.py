...
class DevMarketDataProvider(IMarketDataProvider):
    ...
    async def get_historical_ohlcv(
        ...
        """Deterministic synthetic bars, one per calendar day in
        [start, end] -- each day's bar is generated by the exact same
        _seeded_value(f"{symbol}:{day}", ...) formula get_stock_data
        uses for "today", so a historical backfill and a same-day
        get_stock_data() call agree on that day's values."""
