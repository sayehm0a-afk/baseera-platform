...
class MemoryType(Enum):
    ...
    REFLECTION = "reflection"  # Memories generated by the reflection engine
...
class MemoryStore:
    ...
    def store(
        ...
        # Add to memory
        self.memory[memory_type.value].append(entry)
    ...
    def retrieve(
        ...
        """
        entries = self.memory[memory_type.value]

        # Filter out archived and expired entries
        active_entries = [
            e for e in entries if not e.is_archived and not self._is_expired(e)
        ]
    ...
    def consolidate(self, memory_type: MemoryType) -> None:
        ...
        self.memory[memory_type.value] = unique_entries
    ...
    def _calculate_expiration_time(self, memory_type: MemoryType) -> Optional[datetime]:
        ...
        if memory_type == MemoryType.WORKING:
            ...
            # Use episodic TTL for reflection
            return now + timedelta(days=self.config.episodic_memory_ttl_days)
    ...
    def _is_duplicate(self, entry: MemoryEntry) -> bool:
        ...
        for existing_entry in self.memory[entry.memory_type.value]:
    ...
    def _enforce_size_limits(self, memory_type: MemoryType) -> None:
        ...
        max_entries = {
            MemoryType.WORKING.value: self.config.max_working_memory_entries,
            MemoryType.SHORT_TERM.value: self.config.max_short_term_memory_entries,
            MemoryType.EPISODIC.value: self.config.max_episodic_memory_entries,
            MemoryType.SEMANTIC.value: self.config.max_semantic_memory_entries,
            MemoryType.PROCEDURAL.value: self.config.max_procedural_memory_entries,
            MemoryType.LONG_TERM.value: self.config.max_long_term_memory_entries,
            # Use episodic limit for reflection
            MemoryType.REFLECTION.value: self.config.max_episodic_memory_entries,
        }

        entries = self.memory[memory_type.value]
        if len(entries) > max_entries[memory_type.value]:
            # Remove oldest entries
            entries.sort(key=lambda e: e.timestamp)
            to_remove = len(entries) - max_entries[memory_type.value]

            for entry in entries[:to_remove]:
                if entry.memory_id in self.memory_index:
                    del self.memory_index[entry.memory_id]
            self.memory[memory_type.value] = entries[to_remove:]
    ...
    def delete(self, memory_id: str) -> bool:
        ...
        entry = self.memory_index[memory_id]
        self.memory[entry.memory_type.value] = [
            e for e in self.memory[entry.memory_type.value] if e.memory_id != memory_id
        ]
        del self.memory_index[memory_id]
        logger.debug(f"Memory entry deleted: {memory_id}")
        return True

    def __len__(self) -> int:
        """
        Returns the total number of active memory entries.
        """
