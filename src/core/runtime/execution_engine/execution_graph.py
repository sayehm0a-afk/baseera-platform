...
class IExecutionGraph(ABC):
    ...
    async def execute_graph(
        self, graph_definition: Dict[str, Dict[str, Any]]
    ) -> Dict[str, Any]:
...
class ExecutionGraph(IExecutionGraph):
    ...
    def __init__(
        ...
        dependency_resolver: IDependencyResolver = None,
    ) -> None:
    ...
    async def execute_graph(
        ...
        tasks_for_resolver = {
            task_id: task_data.get("dependencies", [])
            for task_id, task_data in graph_definition.items()
        }
        ...
        for task_id in execution_order:
            if task_id not in graph_definition:
                ...
                logger.warning(
                    "Skipping task %s as it is a dependency but not a defined task in the graph.",
                    task_id,
                )
                continue
            ...
            for dep_id in task_data.get("dependencies", []):
                if dep_id in results:
                    ...
                    # Keep this for now, as the tests expect this format
                    task_kwargs[f"dep_{dep_id}_result"] = results[dep_id]

            try:
                result = await self._executor.execute_task(
                    task_id, task_function, **task_kwargs
                )
