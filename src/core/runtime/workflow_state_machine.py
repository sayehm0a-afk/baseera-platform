"""وحدة Workflow State Machine.

تتولى هذه الوحدة مسؤولية إدارة حالات تدفقات العمل (Workflows) والانتقال بينها
بناءً على الأحداث والنتائج.
"""

import logging
from abc import ABC, abstractmethod
from typing import Any, Dict

logger = logging.getLogger(__name__)


class IWorkflowStateMachine(ABC):
    """واجهة مجردة لـ Workflow State Machine.

    تحدد هذه الواجهة الحد الأدنى من الوظائف المطلوبة لأي تنفيذ لـ Workflow State Machine.
    """

    @abstractmethod
    async def get_current_state(self, workflow_id: str) -> str:  # noqa: E501
        """الحصول على الحالة الحالية لتدفق عمل معين.

        Args:
            workflow_id (str): معرف تدفق العمل.

        Returns:
            str: الحالة الحالية لتدفق العمل.
        """
        raise NotImplementedError

    @abstractmethod
    async def transition_state(
        self,
        workflow_id: str,
        new_state: str,  # noqa: E501
        context: Dict[str, Any] | None = None,
    ) -> None:
        """الانتقال بحالة تدفق عمل معين إلى حالة جديدة.

        Args:
            workflow_id (str): معرف تدفق العمل.
            new_state (str): الحالة الجديدة المراد الانتقال إليها.
            context (Optional[Dict[str, Any]]): سياق إضافي للانتقال.
        """
        raise NotImplementedError


class WorkflowStateMachine(IWorkflowStateMachine):
    """تنفيذ Workflow State Machine.

    مسؤول عن إدارة حالات تدفقات العمل والانتقال بينها.
    """

    def __init__(self) -> None:
        self._workflow_states: Dict[str, str] = {}
        logger.info("WorkflowStateMachine instance created.")

    async def get_current_state(self, workflow_id: str) -> str:  # noqa: E501
        return self._workflow_states.get(workflow_id, "UNKNOWN")

    async def transition_state(
        self,
        workflow_id: str,
        new_state: str,  # noqa: E501
        context: Dict[str, Any] | None = None,
    ) -> None:
        old_state = self._workflow_states.get(workflow_id, "NONE")
        self._workflow_states[workflow_id] = new_state
        logger.info(
            'Workflow "%s" transitioned from %s to %s. Context: %s',  # noqa: E501
            workflow_id,
            old_state,
            new_state,
            context,
        )
