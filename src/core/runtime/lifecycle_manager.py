...
class ILifecycleManager(ABC):
    ...
    @abstractmethod
    async def start_component(self, component_id: str,
...
class LifecycleManager(ILifecycleManager):
    ...
    async def start_component(self, component_id: str,
        ...
        if component_id in self._managed_components:
            logger.warning("Component \'%s\' is already managed and running.", component_id)
            return
    ...
    async def stop_component(self, component_id: str) -> None:
        if component_id not in self._managed_components:
            logger.warning("Component \'%s\' is not managed or not running. "
                           "Nothing to stop.", component_id)
            return
    ...
    async def get_component_status(self, component_id: str) -> Dict[str, Any]:
        ...
        return self._managed_components[component_id] # type: ignore[no-any-return]
