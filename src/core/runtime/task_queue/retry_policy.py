...
class IRetryPolicy(ABC):
    ...
    async def should_retry(
        self, task_id: str, attempt_count: int, error: Exception
    ) -> bool:
        """يحدد ما إذا كان يجب إعادة محاولة مهمة فاشلة.

        Args:
            task_id (str): معرف المهمة.
            attempt_count (int): عدد المحاولات التي تمت حتى الآن.
            error (Exception): الخطأ الذي تسبب في فشل المهمة.

        Returns:
            bool: True إذا كان يجب إعادة المحاولة، False بخلاف ذلك.
        """
    ...
    async def get_delay_seconds(self, task_id: str, attempt_count: int) -> int:
        ...
        Returns:
            int: وقت التأخير بالثواني.
        """
        raise NotImplementedError


class ExponentialBackoffRetryPolicy(IRetryPolicy):
    """تنفيذ سياسة إعادة المحاولة باستخدام التراجع الأسي (Exponential Backoff).

    تزيد وقت التأخير بشكل أسي مع كل محاولة فاشلة.
    """

    def __init__(
        self, max_attempts: int = 3, initial_delay: int = 1, max_delay: int = 60
    ) -> None:
    ...
    async def should_retry(
        ...
        if attempt_count < self._max_attempts:
            ...
            return True
        logger.warning(
            "Task %s (attempt %d) will not retry. Max attempts " "reached. Error: %s",
            task_id,
            attempt_count,
            error,
        )
