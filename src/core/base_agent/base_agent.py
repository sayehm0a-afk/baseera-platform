"""
وحدة BaseAgent الأساسية لمنصة بصيرة.
...
class BaseAgent:
    ...
    def __init__(self, agent_id: Optional[str] = None, name: str = "BaseAgent",
                 description: str = "وكيل ذكي أساسي لمنصة بصيرة"):
...
if __name__ == "__main__":
    ...
    print(f"Initial Status: {agent_example.get_status()}")
    print(f"Agent Info: {agent_example.get_info()}")

    agent_example.activate()
    print(f"Status after activation: {agent_example.get_status()}")
    ...
    agent_example.pause()
    print(f"Status after pausing: {agent_example.get_status()}")

    agent_example.terminate()
    print(f"Status after termination: {agent_example.get_status()}")
