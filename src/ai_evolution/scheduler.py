"""OutcomeEvaluationScheduler: recurring, unattended evaluation of due
`RecommendationOutcome` rows against real forward price data -- the
...
`src.ai_evolution.outcome_evaluation.evaluate_due_outcomes` instead of
a market scan.

Disabled by default (`OUTCOME_EVALUATION_SCHEDULER_ENABLED=false`) --
the same secure/inert-by-default posture every other scheduler in this
...
from src.ai_evolution.config import get_outcome_evaluation_interval_seconds
